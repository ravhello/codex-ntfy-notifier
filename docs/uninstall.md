...
These procedures apply to version 2.4.1. They are intentionally explicit because `~/.codex` also belongs to Codex; never remove that whole directory.
...
Runtime state is not part of the rollback snapshot. Before running substantially older notifier code, move `ntfy-state` to a private, timestamped sibling instead of letting an incompatible version process it. Version 2.4.1 uses record schema 1 and retains the `pending/` and `watch/` state introduced in 2.4.0; compatibility with an arbitrary older build is not guaranteed.
