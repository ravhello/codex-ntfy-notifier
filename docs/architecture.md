...
| AudnCode lifecycle hooks (Windows, opt-in) | Hook shape 8 uses seven synchronous 60-second handlers: `SessionStart`, `UserPromptSubmit`, `Stop`, `StopFailure`, `Notification`, `PostToolUse`, and `SubagentStart`. Exact matchers limit root-session, idle, and tool signals; the tool matcher includes `SendMessage`. |
...
- AudnCode finality depends on seven local shape-8 hook payloads, process/session markers, transcript and command-queue layout, error/team/task records, CCR sidecars, scheduler lease/files, sidechain naming, `idle_prompt` timing, and workspace trust behavior, all of which can change upstream;
