...
La versione 2.6 aggiunge AudnCode su Windows. Uno `Stop` normale è soltanto un candidato e richiede il successivo `Notification: idle_prompt`; `StopFailure` è terminale solo dopo la prova di un unico errore del prompt corrente. La prova salta esclusivamente `idle_prompt`: tutti gli altri gate restano obbligatori. `UserPromptSubmit` pre-arma i prompt ordinari, mentre `SessionStart` crea l'epoca root protetta per piani iniziali diretti e contenuti complessi che AudnCode 0.9.x può avviare senza `UserPromptSubmit`. `SubagentStart` distingue un figlio sincrono nuovo dalla ripresa di un agente locale. L'ordine di avvio dei processi impedisce agli eventi vecchi di liberare lavoro nuovo; `agent_completed` non è prova di finalità. Una `AskUserQuestion` root segue un percorso separato di intervento: soltanto una domanda unica e ancora irrisolta nel transcript attendibile può produrre una notifica compatta, deduplicata anche dopo il resume. La risposta allo stesso tool apre una nuova epoca prima della continuazione; la domanda non viene mai conteggiata come completamento.

I launcher AudnCode gestiti possono inoltre dichiarare un marker privato per tentativo tramite `CODEX_NTFY_AUDNCODE_RECOVERY_MARKER`. La revisione 1 trattiene `StopFailure`; un terminale verificato `recovered` lo annulla, mentre solo `exhausted` per lo stesso UUID di errore può procedere. Su Windows radice recovery, directory manager e marker devono avere ciascuno una DACL protetta, senza ACE ereditate e con accesso soltanto per utente corrente, SYSTEM e Administrators. I lanci AudnCode ordinari restano invariati, ma un launcher che dichiara il contratto non fa fallback se il marker è mancante, malformato, riscritto, con permessi allargati o non correlabile. Non esiste volutamente un TTL temporale: una revisione 1 lunga resta in attesa e una revisione 2 immutabile resta consumabile dopo l'uscita del manager. Un errore recuperato lascia inoltre una tombstone locale permanente legata alla sua identità, perché l'età non rende sicuro un replay tardivo. Il successore `Stop` normale, serializzato esattamente, resta soltanto come journal write-ahead con hash finché uno Stop logicamente identico non è durevole o soppresso in modo terminale. Un indice attivo separato e privo di contenuto limita la riconciliazione ai soli journal vivi. Dopo consegna, soppressione o recovery al riavvio, il successore viene cancellato e l'indice rimosso, così la deduplicazione permanente non conserva per sempre messaggi o percorsi.
...
8. con `-EnableAudnCode`, conserva le voci estranee e installa atomicamente la forma 9 con sette handler sincroni da 60 secondi (`SessionStart`, `UserPromptSubmit`, `Stop`, `StopFailure`, `Notification`, `PostToolUse`, `SubagentStart`), matcher esatti inclusi `permission_prompt` e `AskUserQuestion`, e argomento evento-atteso; imposta la soglia idle a 1.000 ms, gestisce il marker e rimuove soltanto processi hook obsoleti verificati come orfani; un lock di transazione globale alla macchina serializza ogni installer che tocca l'attività pianificata condivisa, poi un lock per home protegge marker AudnCode e rollback compare-and-swap;
